...
1. **Black** - I like to enforce code formatting. Some find this a step too far
   but to me it's not negotiable - developers must format their code if they
   want to contribute to the project.

2. **Pylint** - A linter can check for many problems, but my go-to is always
   the "unused import" rule which ensures nothing is imported that isn't used.

3. **Mypy** - Python is a dynamically typed language but recent versions allow
   type annotations which is better than nothing. A separate tool is needed to
...
deployments unless tests are passing.
...
I recommend requiring these checks to pass before allowing a pull request to be
merged. This can be done in the Github repository's settings under `Settings >
Branches > Branch protection rules`. Click `Add rule`, enter a name under
Branch name pattern ("Checks"), tick "Require status checks to pass before
merging".  Search for "checks" in the search bar, and select it. Then click
`Add rule`.
