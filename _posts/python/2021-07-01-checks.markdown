...
It's more important to have checks in a central location, but still install
these locally to get fast feedback. 
...
Branches > Branch protection rules`. Click `Add rule`, type a Branch name
pattern (e.g. "master"), tick "Require status checks to pass before merging".
Search for "checks" in the search bar, and select it. Then click `Add rule`.
